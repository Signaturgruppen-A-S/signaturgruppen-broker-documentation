...
It is possible to specify multiple identity providers for a single user flow.
...
Signaturgruppen Broker will automatically let the user select the preferred identity provider for the current flow.
As an example, setting the idp_values parameter to “mitid passkeyid” enables the user to login with either MitID or PasskeyID (buttons shown in that order).
